        ...
        with open(path.join(dataset_path, 'labels.csv'), newline='') as f:
            ...
            for fname, label, _ in reader:
                if label in LABEL_NAMES:
                    image = Image.open(path.join(dataset_path, fname))#open image file
                    image.load()
                    label_id = LABEL_NAMES.index(label)#search array index
                    self.datalist.append((image, label_id))
