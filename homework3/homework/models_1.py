...
class FCN(torch.nn.Module):
    ...
    def __init__(self, layers=[16, 32, 64, 128], n_output_channels=5, kernel_size=3, use_skip=True):
        ...
        #for i in L:
        #    print(i)
        self.conv_block=torch.nn.Sequential(*L)
        #print('----------')
        L=[]
        #self.dense_block2 = torch.nn.Sequential()
        for i, l in list(enumerate(layers))[::-1]:
            L.append(self.UpBlock(n_input_cahnnels, l, kernel_size, 2,self.use_skip))
            #self.dense_block2.add_module('dense_conv%d'%i, self.UpBlock(n_input_cahnnels, l, kernel_size, 2,self.use_skip))
            n_input_cahnnels = l
            if self.use_skip:
                n_input_cahnnels += skip_layer_size[i]
        self.dense_block = torch.nn.Sequential(*L)
        #for i in L:
        #    print(i)
        #self.dense_block = torch.nn.Sequential(*L)
    ...
    def forward(self, x):
        ...
        for i in range(self.n_conv_layer):
            ...
            conv_input.append(z)
            z = self.conv_block[i](z)#self.conv_block._modules['blcok_conv%d'%i](z)
            #print(str(z.shape))

        z=self.conv_block(x)
        for i in reversed(range(self.n_conv_layer)):
            z = self.dense_block[self.n_conv_layer-i-1](z,conv_input[i])#self.dense_block2._modules['dense_conv%d'%i](z,conv_input[i])
